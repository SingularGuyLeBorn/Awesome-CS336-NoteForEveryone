...
![Baseline(DDP)模式下模型各部分内存占用比例.](./img_6.png)
...
![PDF 第20页截图, 展示了ZeRO Stage 2的增量式反向传播流程.](img_8.png)
...
![PDF 第22页截图, 简要展示了FSDP在一个FSDP实例(N层)上的工作流程.](img_9.png)
...
![PDF 第23页截图, 详细展示了FSDP中GPU计算流和通信流如何并行工作.](img_10.png)
...
* **实现原理 (以 Megatron-LM 为例)**:
  ...
  * **Attention 层**: 按**注意力头 (Attention Heads)** 进行切分, 每个 GPU 负责一部分头的计算.
  ...
  ![Tensor Parallel in an MLP block](img_21.png)
...
* **特性与适用场景**:
  ...
  * **经验法则**: 张量并行**必须**用于具有极高带宽、极低延迟连接的 GPU 之间. 在实践中, 这意味着它几乎总是被限制在**节点内部 (intra-node)**, 利用 NVLink 等高速总线, 通常并行度为 8 (对应单机 8 卡). 一旦跨越节点边界, 性能会急剧下降.
  ...
  ![TP scaling performance](img_19.png)
...
![Activation memory dominates total memory for large models.](img_23.png)
...
*   **工作流程**: 序列并行并非一个孤立的策略，而是与张量并行（TP）**交替协作**的。它通过在不同计算阶段巧妙地切换数据布局（Data Layout）来实现并行化。
    ...
    ![Sequence Parallelism workflow](img_25.png)
