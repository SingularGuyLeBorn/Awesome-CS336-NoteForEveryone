...
### 2.3 并非总是变好：反向缩放 (Inverse Scaling)
Scaling Laws 暗示了“大就是好”，但在某些特定任务上，模型越大表现反而越差，这就是 **Inverse Scaling**。
*   **原因**: 通常发生在任务目标与预训练数据的“默认行为”冲突时。例如，如果任务要求避免复制输入，但大模型“记忆与复制”的能力更强，它反而表现得更差。
*   **Inverse Scaling Prize**: 这是一个专门收集此类“反直觉案例”的比赛，提醒我们在盲目追求规模时要警惕副作用。
...
1.  **数据混合与代理模型 (Data Mixture & Proxy Models)**：
    ...
    *   **工程启示**: 这意味着我们可以采用**代理模型策略**。你不需要在 100B 模型上测试哪种数据配比最好，只需在一个 1B 甚至更小的模型上测试。因为小模型上 Loss 更低的配比（Offset 更低），在大模型上大概率也是更低的。这是行业内极低成本优化数据的标准范式。
...
*   **临界 Batch Size (Critical Batch Size)**：这是并行化收益递减的转折点。
    *   当 Batch Size < 临界值：增加 Batch Size 相当于增加梯度步数（完美扩展）。
    *   当 Batch Size > 临界值：收益迅速降低，受限于优化景观的曲率。
*   **与 Loss 的关系**：**目标 Loss 越低（模型越强/训练越久），临界 Batch Size 越大**。
    *   这意味着随着训练进行，你应该增大 Batch Size。
    *   LLaMA 3 的训练报告中就体现了这一点：随着 Loss 下降，增加 Batch Size。
*   **物理直觉：噪声与曲率 (Noise vs Curvature)**：
    *   **梯度噪声尺度 (Gradient Noise Scale)** 是核心指标。训练后期，Loss 变小，梯度信号变弱，噪声相对变大。
    *   此时必须增大 Batch Size 来“平滑”噪声，才能获得准确的下降方向。这解释了为什么**目标 Loss 越低（模型越强），需要的临界 Batch Size 越大**。
