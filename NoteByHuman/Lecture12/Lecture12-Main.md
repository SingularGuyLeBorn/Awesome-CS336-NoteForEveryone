...
精英链接笔记 (创建独立文件, 总数不超过5个):
  - **Chatbot Arena 与 ELO 评分系统** [理由: 课堂详细讨论了其工作原理、优缺点及争议; 是当前指令遵循评估的事实标准, 值得深入探讨其方法论]
  - **训练-测试污染 (Train-Test Contamination)** [理由: 课堂多次提及, 是评估有效性的核心挑战; 涉及推断方法和社区规范, 有拓展价值]
  - **Agent 基准: SWE-Bench 与 CyBench** [理由: 代码配套, 是新兴评估范式; 涉及 Agent 架构和评估复杂性]

正文内详述概念 (仅在正文加粗, 不创建链接):
  - 困惑度 (Perplexity) 及其最大化主义者观点
  - MMLU / MMLU-Pro / GPQA / HLE 知识基准
  - IFEval / AlpacaEval / WildBench 指令遵循基准
  - MLE-Bench (Kaggle 竞赛 Agent)
  - ARC-AGI 纯推理基准
  - HarmBench / AIR-Bench 安全基准
  - Jailbreaking (GCG Attack)
  - 真实性问题 (Clio / MedHELM)
--- END OF BLUEPRINT ---
```
